...
def format_loss_accuracy(test_results, ni_neurons, nj_neurons,
...
def output_loss_accuracy(test_results, ni_neurons, nj_neurons, epochs, batch_size, optimizer, stats_dir):
    name = str(ni_neurons) + '_' + str(nj_neurons) + '_' + str(epochs) + '_' + str(batch_size) + '_' + str(optimizer)
    name_of_save_file: str = 'LOSS_ACCURACY_' + str(name) + '.csv'
    chdir(stats_dir)
    test_results.to_csv(name_of_save_file, index=False)


def format_and_output_loss_accuracy(test_results, ni_neurons, nj_neurons, epochs, batch_size, optimizer, stats_dir):
    test_results = format_loss_accuracy(test_results, ni_neurons, nj_neurons, epochs, batch_size, optimizer, stats_dir)
    output_loss_accuracy(test_results, ni_neurons, nj_neurons, epochs, batch_size, optimizer, stats_dir)
