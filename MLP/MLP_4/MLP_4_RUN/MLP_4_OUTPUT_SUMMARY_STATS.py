...
def output_summary_stats(roc_auc, mse, bic, aic, ni_neurons, nj_neurons, nk_neurons, nl_neurons,
    ...
    nk_neurons_df = pd.DataFrame(np.repeat(np.array(nk_neurons), r, axis=0), columns=['NK_NEURONS'])
    nl_neurons_df = pd.DataFrame(np.repeat(np.array(nl_neurons), r, axis=0), columns=['NL_NEURONS'])
    ...
    full_df = pd.concat([roc_auc_df, mse_df, bic_df, aic_df, ni_neurons_df, nj_neurons_df, nk_neurons_df,
                         nl_neurons_df, epochs_df, activation_df, batch_size_df, optimizer_df, num_classes_df,
