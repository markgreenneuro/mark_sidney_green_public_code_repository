...
def randomforest_classifier(x_train, x_test, y_train, y_test, splits):
    clf = RandomForestClassifier(n_estimators=splits, n_jobs=-1)
...
def run_all_random_forest() -> None:
    ...
    [y_test, y_pred, clf] = randomforest_classifier(x_train, x_test, y_train, y_test, splits)
