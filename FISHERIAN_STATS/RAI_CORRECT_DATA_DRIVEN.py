...
def create_reformatted_output(df):
    df = pd.DataFrame(np.array(df).ravel().reshape(334, 20))
    df.columns = df.iloc[0]
    df = df.iloc[1:]
    return df


def write_reformatted_output_data(df, modality):
    # WRITE_REFORMATTED_OUPUT
    df.to_csv(str(modality) + '_reformatted_output.csv', index=None)


def read_reformatted_output_data(root, modality):
    os.chdir(root)
    df = pd.read_csv(str(modality) + '_reformatted_output.csv')
    return df


def remove_bilateral_data(df):
...
def correct_reformatted_output_data(df):
    # DROP AND RENAME
...
def correct_data_driven(root, modality):
    df = open_formatted_output(root, modality)
    df = create_reformatted_output(df)
    write_reformatted_output_data(df, modality)
    df = read_reformatted_output_data(root, modality)
    df = remove_bilateral_data(df)
    df = correct_reformatted_output_data(df)
    write_reformatted_output_data(df, modality)
    df = read_reformatted_output_data(root, modality)
    return df
