...
from MLP_2_CONF_MAT_CLASS_REPORT_FORMAT import *
from MLP_2_OUTPUT_CONF_MAT_CLASS_REPORT import *
from MLP_2_OUTPUT_SUMMARY_STATS import *
from MLP_2_LOSS_ACCURACY_OUTPUT import *
...
def get_params():
    ...
    activation = str(sys.argv[6])
    return [ni_neurons, nj_neurons, epochs, batch_size, optimizer, activation]
...
def get_make_stats_root(results_dir):
    stats_dir = Path.home().joinpath(results_dir, str('MLP_2_STATS'))
    if stats_dir.exists():
        pass
    else:
        makedirs(stats_dir)
...
def build_mlp_2(x_train, stutter_train, ni_neurons, nj_neurons, epochs, batch_size, activation,
    ...
    model.fit(x_train, stutter_train, epochs=epochs, batch_size=batch_size, verbose=1, validation_split=0.2)
    return model


def run_mlp_2(model, x_test, stutter_test, num_classes):
    ...
    class_report = classification_report(np.argmax(stutter_test, axis=1), np.argmax(stutter_preds, axis=1),
                                         zero_divisiion=1, output_dict=True)
...
def set_model_output(output_dir):
    model_dir = Path.home().joinpath(output_dir, 'MLP_2_MODELS')
    if model_dir.exists():
        pass
    else:
        makedirs(model_dir)
...
def run_all_mlp_2() -> None:
    start_time = datetime.now()
    [ni_neurons, nj_neurons, epochs, batch_size, optimizer, activation] = get_params()
    ...
    model = build_mlp_2(x_train, stutter_train, ni_neurons, nj_neurons, epochs, batch_size, activation,
                        optimizer, num_classes, input_shape)
    [test_results, stutter_preds, class_report, conf_mat] = run_mlp_2(model, x_test, stutter_test, num_classes)
    ...
    name = str(ni_neurons) + '_' + str(nj_neurons) + '_' + str(epochs) + '_' + str(batch_size) + '_' + str(
        activation) + '_' + str(batch_size) + '_' + str(optimizer) + '_' + str(activation)
    [class_report, conf_mat] = class_report_conf_mat(conf_mat, class_report, ni_neurons, nj_neurons, epochs, batch_size,
                                                     optimizer)
    output_conf_mat_class_report(name, class_report, conf_mat, stats_dir)
    output_summary_stats(roc_auc, mse, bic, aic, ni_neurons, nj_neurons,
                         epochs, activation, batch_size, optimizer, num_classes, input_shape, name, stats_dir)
    format_and_output_loss_accuracy(test_results, ni_neurons, nj_neurons, epochs, batch_size, optimizer, stats_dir)
    model_dir = set_model_output(results_dir)
    save_model(model_dir, model, name)
    time_delta = datetime.now() - start_time
    exit_message = 'MLP 2 RAN SUCCESSFULLY IN: ' + str(time_delta)
